"""
GraSP: Gradient Signal Preservation pruning (Wang et al., 2020).
"Picking Winning Tickets Before Training by Preserving Gradient Flow".

Original implementation from: GraSP_Original/GraSP/pruner/GraSP.py

Usage:
    keep_masks = GraSP(net, ratio, train_dataloader, device,
                       num_classes=10, samples_per_class=25)
    # `ratio` is the fraction of weights to PRUNE.
    # keep_masks is a dict {module: binary_mask}. 1 = keep, 0 = prune.

GraSP scores each weight by its effect on the gradient norm after pruning,
using a Hessian-gradient product (-theta * (H g)), and removes the weights that
most increase the loss / damage gradient flow.
"""
